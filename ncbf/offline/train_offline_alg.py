...
from og.jax_types import BBFloat, FloatDict, Shape
from og.jax_utils import jax_jit_np, jax_vmap, merge01
...
from ncbf.avoid_utils import get_max_gae, get_max_gae_term, get_max_mc
...
class TrainOfflineAlg(struct.PyTreeNode):
    Cfg = TrainOfflineCfg
    ...
    def create(cls, key: PRNGKey, obs_mean, obs_std, nh: int, cfg: TrainOfflineCfg):
        ...
        return TrainOfflineAlg(zero, key, value_net, ema, obs_mean, obs_std, cfg)

    def _compute_targets(self, b_traj: Traj) -> "TrainOfflineAlg.Batch":
    ...
    def update(self, b_traj: Traj):
        ...
        def updates_body(alg_: TrainOfflineAlg, b_batch: TrainOfflineAlg.Batch):
    ...
    def _update_value(self, batch: Batch) -> tuple["TrainOfflineAlg", FloatDict]:
    ...
    @ft.partial(jax.jit)
    def eval(self, T_obs_eval: TObs, Th_h_eval: THFloat, counter_clock_wise = 0) -> EvalData:
        # Visualize the learned value function.
        # [ vx, vy, wz, wF, wR, Psi, x, y] # TODO: what is a good default value for x0?
        x0 = np.array([7.0, 0.0, 0.0, 65.0, 85.0, 0.0, 0.0, 0.0])
        ...
        bb_s, bb_ey = jnp.meshgrid(b_s, b_ey)

        b1, b2 = bb_s.shape
        assert (b1, b2) == (n_ey, n_s)

        bb_epsi = jnp.zeros_like(bb_s) + counter_clock_wise * jnp.pi
        bb_state = ei.repeat(x0, "nx -> b1 b2 nx", b1=b1, b2=b2)
        bb_state_curvi = jnp.stack([bb_epsi, bb_ey, bb_s], axis=-1)

        obs_cfg, h_cfg = get_cfgs()

        #TODO: what is rep here?
        bb_obs = jax_vmap(ft.partial(state_to_obs, track, obs_cfg), rep=2)(bb_state, bb_state_curvi)
        bbh_h = jax_vmap(ft.partial(get_h_vector, h_cfg), rep=2)(bb_state, bb_state_curvi)
        ...
        # For plotting convenience, compute the corresponding cartesian coords.
        with jax.ensure_compile_time_eval():
            b_x, b_y, b_theta, _, _ = jax_jit_np(jax_vmap(track.get_lerp_from_s))(b_s)
            b_pos = np.stack([b_x, b_y], axis=1)
            b_dpos = np.stack([np.cos(b_theta + np.pi / 2), np.sin(b_theta + np.pi / 2)], axis=1)

            bb_pos = b_pos + b_ey[:, None, None] * b_dpos
            assert bb_pos.shape[:2] == (b1, b2)
